...
export const AddNewItem = ({ isEditMode }) => {
      ...
      setTimeout(() => {
        ...
        fetch(`${process.env['REACT_APP_API_URL']}product/${id}?f=1234`, {
          ...
          .then((data) => {
            ...
            const itemColorPhotos = data?.colors?.find(
              (item) => item.link === data.product.link_name,
            );
            if (itemColorPhotos) {
              currentPhotosToSet = {
                ...itemColorPhotos,
  ...
  useEffect(() => {
    if (!!manufacturers.length && currentProduct && allSubcategories.length && !!currentProduct) {
      const manufacturerName = manufacturers?.find(
        (item) => item.id === currentProduct.product.manufacturerId,
      )?.name;

      setCurrentProductId(currentProduct?.product.id);
      setNewItemTitle(currentProduct?.product.name);
      setMemories(
        Array.isArray(currentProduct?.product.Memory)
          ? currentProduct?.product.Memory
          : [currentProduct?.product.Memory],
      );
      ...
      const sortedItemDescriptions = currentProduct?.product.Information.sort(
      ...
      if (productSubcategory?.parentId) {
        const subCategory = allSubcategories.find(
          (item) => item.id === productSubcategory.parentId,
        )?.name;
        setCurrentSelectedSubcategory(subCategory);

        const subsubcategory = allSubcategories.find(
          (item) => item.id === productSubcategory.id,
        )?.name;
      ...
      setSpecialOffer(
        currentProduct?.product?.stickers.find((item) => item.name === 'Специальное предложение'),
      );
    }
    ...
    if (selectedCategory) {
      fetch(
        `${process.env['REACT_APP_API_URL']}subcategory/${
          subCategories?.find((item) => item.name === currentSelectedSubcategory)?.id
        }`,
      )
        .then((res) => res.json())
        .then((data) => {
  ...
  const saveNewItem = (event) => {
    ...
    const descriptionToPost = itemDescriptions.map((item, index) =>
      Object.assign(item, {
    ...
    if (!!newManufacturer?.trim().length || !!currentManufacturer?.trim().length) {
      formData.append(
        'manufacturer',
        newManufacturer?.trim().length ? newManufacturer : currentManufacturer,
      );
    }
    ...
    axios
      .post(`https://dreamstore.by/api/product`, formData, {
  ...
  const saveEdits = () => {
    ...
    let editRequest = new Request(
      `${process.env['REACT_APP_API_URL']}product/${currentProductId}?f=1234`,
      {
        body: formEditData,
        headers: {
          Authorization: token,
  ...
  const generalContent = (
    <div className={`${DEFAULT_CLASSNAME}_general`}>
      ...
      <div className={`${DEFAULT_CLASSNAME}_general_product_info`}>
        ...
        <div className={`${DEFAULT_CLASSNAME}_general_product_info_item`}>
          ...
          <select
            style={{ width: '30% ' }}
            onChange={(e) =>
              e.currentTarget.value === 'Подкатегория'
                ? setCurrentSelectedSubcategory(null)
                : setCurrentSelectedSubcategory(e.currentTarget.value)
            }>
            ...
            {subCategories?.map((item) => (
              <option
                ...
                selected={item.name === currentSelectedSubcategory}
                value={item.name}>
                {item.name}
              </option>
            ))}
          ...
          <select
            style={{ width: '30% ' }}
            onChange={(e) =>
              e.currentTarget.value === 'Под Подкатегория'
                ? setCurrentSubSubCategory(null)
                : setCurrentSubSubCategory(e.currentTarget.value)
            }>
            <option value={null} defaultChecked={true}>
              {'Под Подкатегория'}
            </option>
            {subSubCategories?.map((item) => (
              <option
                key={item.name.toString()}
                selected={item.name === currentSubSubCategory}
                value={item.name}>
                {item.name}
              </option>
            ))}
        ...
        <div className={`${DEFAULT_CLASSNAME}_general_product_info_item`}>
          ...
          <input
            style={{ width: '73%' }}
            className={`${DEFAULT_CLASSNAME}_info_specs_manufacturer`}
            value={newManufacturer}
            onChange={(e) => setNewManufacturer(e.currentTarget.value)}
            type={'text'}
            placeholder={'Введите производителя или выберите из списка'}
          />
          <select onChange={(e) => setCurrentManufacturer(e.currentTarget.value)}>
            {manufacturers.map((item) => {
              return (
                <option
                  key={item.toString()}
                  selected={item.name === currentManufacturer}
                  value={item.name}>
                  {item.name}
                </option>
              );
        ...
        <div className={`${DEFAULT_CLASSNAME}_general_product_info_item`}>
          ...
          <input value={price} onChange={(e) => setPrice(e.currentTarget.value)} type={'text'} />
          <div className={`${DEFAULT_CLASSNAME}_general_product_info_item_small`}>
            <label>$</label>{' '}
            <input
              checked={USDCurrency}
              onChange={() => setUSDCurrency(!USDCurrency)}
              type={'checkbox'}
            />
          </div>
        </div>
      ...
      <div className={`${DEFAULT_CLASSNAME}_general_product_info`}>
        <div className={`${DEFAULT_CLASSNAME}_general_product_info_title`}>
        ...
        <div className={`${DEFAULT_CLASSNAME}_general_product_info_item`}>
          ...
          <input
            ...
            placeholder={'Введите ссылку-имя товара'}
          />
        </div>
        <div className={`${DEFAULT_CLASSNAME}_general_product_info_item`}>
          ...
          <input
            ...
            onChange={(e) => setNewPopularItem(e.currentTarget.value)}
            placeholder={'Введите ссылку-имя товара'}
          />
          <img
            alt={'lazy-im'}
          ...
          {popularItems.map((item) => (
            <div
              onClick={() => {
                ...
                const itemsToSet = [
                  ...popularItems.slice(0, deleteItemIdx),
                  ...popularItems.slice(deleteItemIdx + 1),
                ];

                setPopularItems(itemsToSet);
              }}
              key={item}
              className={'popular_items_item'}>
              {item}
            </div>
          ))}
        </div>
      ...
      <input
        onClick={isEditMode ? saveEdits : saveNewItem}
        type={'button'}
        className={`${DEFAULT_CLASSNAME}_save-item`}
        ...
        <div
          ...
          <div style={{ display: 'flex', flexDirection: 'row', gap: '12px' }}>
            ...
            <div
              ...
              onClick={() => (isEditMode ? deleteMemory(size, id) : deleteMemory(size))}>
              Удалить
            </div>
          </div>
        </div>
      ...
      <div className={`${DEFAULT_CLASSNAME}_general_product_info_item_image`}>
        <div className={`${DEFAULT_CLASSNAME}_general_product_info_item_image_index`}>
          <input
            type={'number'}
            value={priorityValue}
            onChange={(e) => setPriorityValue(e.currentTarget.value)}
          />
  ...
  const ColorComponent = (props) => {
    ...
    return (
      <>
        ...
        <div
          className={`${DEFAULT_CLASSNAME}_general_product_info_item`}
    ...
    <div className={`${DEFAULT_CLASSNAME}_metadata`}>
      ...
      <div className={`${DEFAULT_CLASSNAME}_colors`}>
        <div className={`${DEFAULT_CLASSNAME}_memory_title`}>
          ...
          <div
            className={`${DEFAULT_CLASSNAME}_memory_add`}
            onClick={async () => {
              if (isEditMode) {
                const token = sessionStorage.getItem('admin-dream-token');
        ...
        <div
          className={`${DEFAULT_CLASSNAME}_general_product_info`}
          ...
          <div
            ...
            {!!currentColorPhotos.length &&
              currentColorPhotos.map((item) => (
                <div className={`${DEFAULT_CLASSNAME}_general_product_info_item_image`}>
                  <img
                  ...
                  <div
                    onClick={() => {
                      const deleteItem = currentColorPhotos.findIndex(
                        (image) => image.name === item.name,
                      );
                      setCurrentColorPhotos([
                        ...currentColorPhotos.slice(0, deleteItem),
                        ...currentColorPhotos.slice(deleteItem + 1),
                      ]);
                    }}
                    style={{
                      cursor: 'pointer',
            ...
            currentColors.map((color) => (
              <>
                ...
                <div
                  ...
                  {!!color?.img_path?.length &&
                    color?.img_path?.map((item) => {
                      return (
                        <div className={`${DEFAULT_CLASSNAME}_general_product_info_item_image`}>
                          <img
                            style={{ objectFit: 'contain', borderRadius: '12px' }}
                            alt={item}
                            src={isEditMode ? item : URL.createObjectURL(item)}
                          />
                          <div
                            onClick={() => {
                              const deleteItem = currentColorPhotos.findIndex(
                ...
                <div
                  onClick={() => {
                    ...
                    setCurrentColors([
                      ...currentColors.slice(0, deleteItem),
                      ...currentColors.slice(deleteItem + 1),
                    ]);
                  }}
                  style={{
                    alignSelf: 'flex-end',
      ...
      <div className={`${DEFAULT_CLASSNAME}_memory`}>
        <div className={`${DEFAULT_CLASSNAME}_memory_title`}>
          ...
          <div className={`${DEFAULT_CLASSNAME}_memory_add`} onClick={() => saveNewMemoryHandler()}>
            Добавить
          </div>
        </div>
        <div
          ...
          <div className={`${DEFAULT_CLASSNAME}_general_product_info_item`}>
            <label>Объём:</label>
            <input
              value={newMemoryAmount}
              onChange={(e) => setNewMemoryAmount(e.currentTarget.value)}
              type={'text'}
          ...
          <div className={`${DEFAULT_CLASSNAME}_general_product_info_item`}>
            ...
            <input
              value={newMemoryLink}
              onChange={(e) => setNewMemoryLink(e.currentTarget.value)}
              type={'text'}
              placeholder={'Введите Ссылку'}
            />
          </div>
          <div className={`${DEFAULT_CLASSNAME}_general_product_info_item`}>
            ...
            <input
              value={newMemoryCost}
              onChange={(e) => setNewMemoryCost(e.currentTarget.value)}
              type={'text'}
    ...
    <div className={`${DEFAULT_CLASSNAME}_metadata`}>
      ...
      <div className={`${DEFAULT_CLASSNAME}_metadata_item`}>
        ...
        <textarea
          ...
          placeholder={'Введите краткое описание товара...'}></textarea>
      </div>
    </div>
  );
        ...
        <form onSubmit={saveNewItem}>
          <div className={`${DEFAULT_CLASSNAME}_item-info`}>
            <div className={`${DEFAULT_CLASSNAME}_sections`}>
              {SECTIONS.map((item) => (
                <div
                  className={`${DEFAULT_CLASSNAME}_sections_item ${
                    activeMenuItem === item && 'active-section-item'
                  }`}
