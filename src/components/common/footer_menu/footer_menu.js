...
export const FooterMenu = ({ setIsMobileMenuOpened }) => {
    ...
    const handlers = useSwipeable({ onSwipeStart: () => setSwipeStarted(true), onSwiped: (e) => handleSwipe(e) })

    return (
        <>
            <div className={`${DEFAULT_CLASSNAME}_search ${!isSearchOpened && 'footer-menu-search-closed'}`}>
                <img className={`${swipeStared && 'search-close-zoomed'}`} {...handlers} alt={'close-search'} src={closeSearch} />
                <div className={`${DEFAULT_CLASSNAME}_search_input`}>
                    <input placeholder={'Поиск...'} value={searchText} onChange={(e) => setSearchText(e.currentTarget.value)} type={'text'} />
                ...
                {!isSearchDataLoading && searchText.length >= 1 && isSearchOpened && <div className={`${DEFAULT_CLASSNAME}_searched-items`}>
                    ...
                    {!isSearchDataLoading && searchData?.map(item => {
                        ...
                        return (
                            <div className={`${DEFAULT_CLASSNAME}_searched-items_item`} onClick={(event) => handleNavToItem(event, itemLink, itemCategory, itemSubcategory)}>
                                <div><img alt={'product-photo'} src={item?.img_path?.includes('http') ? item?.img_path : `http://194.62.19.52:7000/${item?.img_path}`}/></div>
                                <div className={`${DEFAULT_CLASSNAME}_searched-items_item_content`}>
                                    ...
                                    <div className={`${DEFAULT_CLASSNAME}_searched-items_item_footer`}>
                                        <span className={`${DEFAULT_CLASSNAME}_searched-items_item_price`}>{item.price === 0 ? "Уточните" : item.price} {item.price !== 0 && "BYN"}</span>
            ...
            <div className={DEFAULT_CLASSNAME}>
                {FOOTER_MENU_ITEMS.map(({ title, link, image }) =>
                    <div onClick={!!link.length ? () => navigate(link)
                                       : title === "Меню" ? () => setIsMobileMenuOpened(true) : () => setIsSearchOpened(!isSearchOpened)
