    ...
    return (
      <>
        ...
        <div style={{ marginTop: '32px', textAlign: 'center' }}>
          Выберите товары для сравнения в
          <span
            onClick={() => navigate('/catalog')}
            style={{ cursor: 'pointer', color: '#FF693EFF' }}>
            {' '}
            Каталоге
          </span>
        </div>
                      ...
                      <>
                        ...
                        {item.value?.map((item2) => (
                          <div className={`${DEFAULT_CLASSNAME}_subitem`}>
                            ...
                            <div
                              className={`${DEFAULT_CLASSNAME}_values ${
                                compareItems.length === 1 && 'only-one-item'
                              }`}>
                              {compareItems.map((it, index) => (
                                <div>{item2.value[index] ?? 'Нет данных'}</div>
                              ))}
                            </div>
                          </div>
                        ))}
