    ...
    <div className={`${DEFAULT_CLASSNAME}_wrapper`}>
      ...
      <div className={DEFAULT_CLASSNAME}>
        ...
        <div className={`${DEFAULT_CLASSNAME}_content`}>
          {!!favoriteItems.length && <div className={`${DEFAULT_CLASSNAME}_clear`} onClick={() => {
            setFavoriteItems([]);
            localStorage.removeItem('favoriteItems');
          }}>{'Очистить'}</div>}

          {!favoriteItems && <div className={`${DEFAULT_CLASSNAME}_no-content`}>Нет избранных товаров</div>}
