...
export const FavoriteItems = ({ setFavoriteItems, favoriteItems, setCartItems }) => {
  return (
      ...
      <div className={DEFAULT_CLASSNAME}>
        ...
        <div className={`${DEFAULT_CLASSNAME}_content`}>
          {!!favoriteItems.length && (
            <div
              className={`${DEFAULT_CLASSNAME}_clear`}
              onClick={() => {
                setFavoriteItems([]);
          ...
          {!favoriteItems && (
            <div className={`${DEFAULT_CLASSNAME}_no-content`}>Нет избранных товаров</div>
          )}

          {!!favoriteItems &&
            favoriteItems?.map((item) => {
              const categoryName = objReplacer[item.category.categoryName];
              // BE NEED TO SEND SUBCATEGORY NAME
              // const subcategory = item.subcategory.link_name;

              return (
                <ItemCard
                  product={item}
                  clickLink={`${categoryName}`}
