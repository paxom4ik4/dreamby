...
export const ServicesFilter = ({
  ...
  useEffect(() => {
    fetch(
      `${process.env['REACT_APP_API_URL']}product/filters?category=${
        selectedSubSubcategory?.id || selectedSubcategory || selectedCategory
      }`,
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <div className={`${classname} ${!catalogFilterOpened && 'servies_filter_hidden'}`}>
      <div className={`${classname}_filter`} style={{ fontWeight: '500' }}>
        Фильтр по продуктам
      </div>
      ...
      <div className={`${classname}_price`}>
        ...
        <div className={`${classname}_price_wrapper`}>
          <input
            value={price.from}
            onInput={fromPriceHandler}
            type={'number'}
            className={`${classname}_price_input`}
            placeholder={'от'}
          />
          <input
            value={price.to}
            onInput={toPriceHandler}
            type={'number'}
            className={`${classname}_price_input`}
            placeholder={'до'}
          />
      ...
      <div className={`${classname}_manufacturer`}>
        ...
        <input
          className={'manufacturer-input-keyword'}
          type={'text'}
          value={keyWord}
              ...
              <input
                ...
                onChange={() =>
                  selectedSubcategories.includes(item)
                    ? deleteItem(selectedSubcategories, setSelectedSubcategories, item)
                    : addItem(selectedSubcategories, setSelectedSubcategories, item)
                }
                type={'checkbox'}
                value={item.id}
              />
      ...
      {!!categoryFilters?.manufacts.filter(Boolean).length && (
        <div className={`${classname}_manufacturer`}>
          <div className={`${classname}_manufacturer_title`}>{'Производитель'}</div>
          {categoryFilters.manufacts
            ...
            .map((item) => (
              <div key={item.toString()} className={`${classname}_manufacturer_item`}>
                <input
                  ...
                  onChange={() =>
                    selectedManufacturers.includes(item)
                      ? deleteItem(selectedManufacturers, setSelectedManufacturers, item)
                      : addItem(selectedManufacturers, setSelectedManufacturers, item)
                  }
                  type={'checkbox'}
                  value={item}
                />
      ...
      {!!categoryFilters?.memory.filter(Boolean).length && (
        <div className={`${classname}_manufacturer`}>
          <div className={`${classname}_manufacturer_title`}>
            {'Память'}
          ...
          <div className={'colors_container'}>
            {[...new Set(categoryFilters.memory.map((item) => item.trim()))]
              .filter(Boolean)
              .sort((a, b) => a - b)
              .map((item) => (
                <div
                  onClick={() =>
                    selectedMemory.includes(item)
                      ? deleteItem(selectedMemory, setSelectedMemory, item)
                      : addItem(selectedMemory, setSelectedMemory, item)
                  }
                  key={item.toString()}
                  className={`${classname}_memory_item ${
                    selectedMemory.includes(item) && 'selected_color'
                  }`}>
                  {item}
                </div>
              ))}
      ...
      <div className={`${classname}_manufacturer`}>
        <div className={`${classname}_manufacturer_title`}>
          {'Сортировка'}
          {!!(sortingOrder || alphabetSort || sortingInStock === false || sortingInStock) && (
            <span
              onClick={() => {
        ...
        <form>
          ...
          {catalogFilterOpened && (
            <div onClick={() => setCatalogFilterOpened(false)} className={'confirm-filter-btn'}>
              Применить
            </div>
          )}
        </form>
  ...
  return (
    <div className={DEFAULT_CLASSNAME}>
      ...
      <div style={{ display: 'none' }} itemScope itemType="https://schema.org/Store">
        ...
        Телефон:{' '}
        <span itemScope itemProp="telephone">
          +37529 155 30 20
        </span>
      ...
      <PopularItems
        ...
        favoriteItems={favoriteCatalogItems}
        currentRate={currentRate}
        setCartItems={setCartItems}
